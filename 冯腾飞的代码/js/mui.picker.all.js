	...
	(function($, document) {
		...
		var PopPicker = $.PopPicker = $.Class.extend({
			...
			init: function(options) {
				...
				self.ok.addEventListener('tap', function(event) { //修改:by feng
					if(self.callback) {
			...
			//增加:绑定默认值
			//by feng
			setValue: function(layerNumber, values) {
				var self = this;
				if(self.pickers.length > layerNumber && values.length > layerNumber) {
					self.pickers[layerNumber].setSelectedValue(values[layerNumber], 0, function() {
						self.setValue(layerNumber + 1, values);
					});
				}
			},
			//填充数据
			//修改:添加一个values数组,用于添加默认值. by feng
			setData: function(data, values) {
				var self = this;
				data = data || [];
				self.pickers[0].setItems(data);
				if(values) {
					self.setValue(0, values);
				}
			},
			//获取选中的项（数组）
			...
			hide: function(flg) {
				...
				if(!flg && typeof self.callback == 'function') {
					var close = self.callback([]);
					if(close === false) { //不允许关闭
						return;
					}
				}
