define(['js/mui', 'zoom'], function($) {
	...
	proto.addImage = function(img, defaultGroup) {
		var group = defaultGroup || img.getAttribute('data-preview-group');
	...
	proto.openImage = function(img) {
		...
		group = group || defaultGroupName;
		this.groups[group] = [];
		var imgs = document.querySelectorAll("img[data-preview-src][data-preview-group='" + group + "']");
		for(var i = 0, len = imgs.length; i < len; i++) {
			this.addImage(imgs[i]);
		}
		this.openByGroup(this.groups[group].indexOf(img.__mui_img_data), group);
	};

	//只打开单张图片预览(by feng)
	proto.openImageOnly = function(img) {
		var self = this;
		if(self.isShown()) {
			return;
		}
		self.groups[ONLY_IMAGE] = []; //清空图片
		self.addImage(img, ONLY_IMAGE); //增加单张图片
		this.openByGroup(this.groups[ONLY_IMAGE].indexOf(img.__mui_img_data), ONLY_IMAGE);
	};
	...
	var previewImageApi = new PreviewImage();
	...
	return {
		...
		/**
		 * 开启图片预览,img 节点必须有两个属性:
		 * data-preview-src : 大图的路径(如果值为空取src的路径)
		 * data-preview-group : 1,2,3,4,5..... 相同的值会一起让用户左右滑动显示
		 * @param {Object} img Dom 节点
		 * @param {Object} options
		 */
		openImage: function(img, options) {
			if(options) this.init(options);
			img = img.querySelector('img') || img;
			var group = img.getAttribute('data-preview-group');
			if(group == '' || group == null) {
				console.log('必须设置data-preview-group属性的值');
				return;
			}
			previewImageApi.openImage(img);
		},
		openImageOnly: function(img, options) { //只预览单张图片
			if(options) this.init(options);
			img = img.querySelector('img') || img;
			previewImageApi.openImageOnly(img);
